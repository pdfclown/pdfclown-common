...
`v.0.0.0` marks a special commit (**CHANGELOG history base**) which provides a clean start for CHANGELOG generated by commitizen (`cz changelog`) on first release.

> [!NOTE]
> The CHANGELOG history base tag (`v.0.0.0`) is configured via `changelog_start_rev` property at `.cz.toml` in the project.
