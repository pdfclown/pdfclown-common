/*
  ...
  This file (JSONCompareTest.java) is part of pdfclown-common-build module in pdfClown Common
...
public class JSONCompareTest {
  ...
  public void reportsUnexpectedFieldInNonExtensibleMode() throws JSONException {
    JSONCompareResult result =
        compareJSON("{\"obj\": {}}", "{\"obj\": {\"id\": 3}}", NON_EXTENSIBLE);
    assertThat(result, failsWithMessage(equalTo("obj\nUnexpected: id\n")));
    assertEquals(result.getFieldUnexpected().size(), 1);
  }
  ...
  public void reportsUnmatchedJSONArrayWhereExpectedContainsJSONObjectWithUniqueKeyButActualContainsElementOfOtherType()
    ...
    assertThat(result,
        failsWithMessage(equalTo("[0] Could not find match for element {\"id\":3}")));
  }
  ...
  public void reportsUnmatchedJSONObjectInUnorderedArray() throws JSONException {
    JSONCompareResult result = compareJSON("[{\"address\" : {\"street\" : \"Acacia Avenue\"}}]",
        "[{\"age\" : 23}]", LENIENT);
    assertThat(result, failsWithMessage(equalTo(
        "[0] Could not find match for element {\"address\":{\"street\":\"Acacia Avenue\"}}")));
  }
  ...
  private Matcher<JSONCompareResult> failsWithMessage(final Matcher<String> expectedMessage) {
    return new TypeSafeMatcher<>() {
      ...
      public void describeTo(Description description) {
        description.appendText("a failed comparison with message ")
